...
class Block(Rectangle):
    ...
    def __init__(self, pos, color):
        ...
        '''
        # those with 'self' keyword are attribute
        self.x = pos.x
        self.y = pos.y
        
        # p1, p2 are local variable
...
class Board(object):
    ...
    def __init__(self, win, width, height):
        ...
        for i in range (self.width):
        	for j in range (self.height):
        		self.block_list[(i,j)] = Block(Point(i,j), 'red')
    ...
    def get_block_neighbors(self, block):
        ...
        for x_offset in offsets:
            for y_offset in offsets:
                if (x_offset + x, y_offset + y) in self.block_list.keys() and (x_offset != 0 or y_offset != 0):
                    neighbor.append(self.block_list[(x_offset + x, y_offset + y)])
        return neighbor
    ...
    def simulate(self):
        ...
        for block in self.block_list.values():
            neighbors_list = self.get_block_neighbors(block)
            live_neighbor = self.test_live_neighbor(neighbors_list)
            if block.is_live():
                ...
                if (live_neighbor == 3):
                    ...
                    block.new_status = 'dead'
        # reset block status after update all of them
        for block in self.block_list.values():
            block.reset_status(self.canvas)


    def test_live_neighbor(self, neighbors_list):
        '''
        #Given a list of neighboring blocks, return those status == 'live'
        '''
        live_neighbor = 0
